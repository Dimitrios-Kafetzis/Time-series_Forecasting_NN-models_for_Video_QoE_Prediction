...
# UPDATED: Modified to work with timeseries_forecasting_models_v6.py which supports separate
# train, validation, and test directories and a configurable output directory

# Define data directories
TRAIN_DIR="./final_complete_dataset/train_set"     # Directory containing training data
VAL_DIR="./final_complete_dataset/validation_set"  # Directory containing validation data
TEST_DIR="./final_complete_dataset/test_set"       # Directory containing test data

# Define the output directory for models
OUTPUT_DIR=~/Impact-xG_prediction_model/forecasting_models_v6_complete_dataset
...
echo "Using data from:" | tee -a "$LOG_FILE"
echo "  - Training:    $TRAIN_DIR" | tee -a "$LOG_FILE"
echo "  - Validation:  $VAL_DIR" | tee -a "$LOG_FILE"
echo "  - Testing:     $TEST_DIR" | tee -a "$LOG_FILE"
...
train_model() {
    ...
    local cmd="python3 timeseries_forecasting_models_v6.py \
                --train_dir $TRAIN_DIR \
                --val_dir $VAL_DIR \
                --test_dir $TEST_DIR \
