#!/bin/bash
# test_linear_model.sh
# A simplified script to test just the linear_basic.h5 model and generate feature importance data

# Define paths
MODELS_DIR=~/Impact-xG_prediction_model/forecasting_models_v5
DATA_FOLDER=~/Impact-xG_prediction_model/real_dataset
SCALER_FILE="$MODELS_DIR/scaler.save"
SEQ_LENGTH=5
REPORT_FILE="$MODELS_DIR/linear_model_report.txt"
RESULTS_CSV="$MODELS_DIR/linear_model_results.csv"
FEATURE_IMPORTANCE_DIR="$MODELS_DIR/feature_importance"
MODEL_FILE="$MODELS_DIR/linear_basic.h5"

echo "Starting test for linear_basic.h5 model..."

# Create directories if they don't exist
mkdir -p "$FEATURE_IMPORTANCE_DIR"

# Create a simplified version of debug_test.py for this model
cat > test_linear_debug.py << 'PYTHON_EOF'
#!/usr/bin/env python3
"""
...
export SCALER_FILE="$SCALER_FILE"
export MODEL_FILE="$MODEL_FILE"
export FEATURE_IMPORTANCE_DIR="$FEATURE_IMPORTANCE_DIR"
export RESULTS_CSV="$RESULTS_CSV"

# Run the Python script
python3 test_linear_debug.py
...
echo "=======================================================" > "$REPORT_FILE"
echo "        LINEAR MODEL EVALUATION REPORT                 " >> "$REPORT_FILE"
echo "        Generated on $(date)                           " >> "$REPORT_FILE"
echo "=======================================================" >> "$REPORT_FILE"
echo "" >> "$REPORT_FILE"
echo "Model: $(basename "$MODEL_FILE")" >> "$REPORT_FILE"
echo "Dataset: $DATA_FOLDER" >> "$REPORT_FILE"
echo "Sequence Length: $SEQ_LENGTH" >> "$REPORT_FILE"
echo "" >> "$REPORT_FILE"

# Read metrics from the CSV file
model_metrics=$(tail -n 1 "$RESULTS_CSV")
model_name=$(echo "$model_metrics" | cut -d ',' -f 1)
model_type=$(echo "$model_metrics" | cut -d ',' -f 2)
mse=$(echo "$model_metrics" | cut -d ',' -f 3)
mae=$(echo "$model_metrics" | cut -d ',' -f 4)
r2=$(echo "$model_metrics" | cut -d ',' -f 5)
latency=$(echo "$model_metrics" | cut -d ',' -f 6)

# Add metrics to the report
echo "Model Type: $model_type" >> "$REPORT_FILE"
...
feature_imp_file="$FEATURE_IMPORTANCE_DIR/${model_name}.importance.json"
if [ -f "$feature_imp_file" ]; then
    echo "Feature Importance:" >> "$REPORT_FILE"
    
    # Check if jq is available
    if command -v jq >/dev/null 2>&1; then
        # Extract top 10 features
        echo "Top 10 Features:" >> "$REPORT_FILE"
        jq -r '.feature_importance[:10] | .[] | "  \(.feature): \(.weight)"' "$feature_imp_file" >> "$REPORT_FILE"
        echo "" >> "$REPORT_FILE"
        
        # Extract bias term
        bias=$(jq -r '.bias' "$feature_imp_file")
        echo "Bias term: $bias" >> "$REPORT_FILE"
    else
        # Simple extraction without jq
        echo "Feature importance data is available in $feature_imp_file" >> "$REPORT_FILE"
        echo "(Install 'jq' for detailed feature display in this report)" >> "$REPORT_FILE"
    fi
fi
...
echo "Feature importance data saved to $feature_imp_file"